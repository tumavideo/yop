...
describe('Visit Inlight and click "Find an opportunity"', () => {
  it('clicks the link "Find an opportunity"', () => {
    // Set viewport dimensions (replace 'Width' and 'height' with actual values, e.g., 1280, 720)
    cy.viewport(1280, 720);

    // Visit the site (replace 'site' with the actual URL you want to test)
    cy.visit(site);

    // Click on the link containing the text 'Find an opportunity'
    cy.contains('Find an opportunity').click();

    // Check for text indicating a successful navigation or action
    cy.contains('Sign up to start your journey');
