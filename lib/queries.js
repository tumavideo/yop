...
export const findOpportunities = (count = 500) => `{
  "job": *[_type == "job" && dateTime(_updatedAt) > dateTime(now()) - 60*60*24*10 && !(_id in path('drafts.**'))]{_id,_createdAt,position,description,experience,field,location,qualifications,closingDate,enableApply,link,responsibilities,role,slug,title,companyRef->{_id,company,bio,logo,website}} | order(_createdAt desc)[0..${count}],
  ...
  "fields": *[_type == "job" && dateTime(_updatedAt) > dateTime(now()) - 60*60*24*10 && !(_id in path('drafts.**')) && defined(field)]{field}
}`;
